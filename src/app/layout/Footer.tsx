    ...
    <footer className="bg-black text-white">
      ...
      <div className="mx-auto max-w-7xl px-6 py-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2 text-sm">
            <li>
              <span className="font-semibold">{t("footer.contact.location_label")}</span>{" "}
              {t("footer.contact.location_value")}
            </li>
            <li>
            ...
            <li>
              <span className="font-semibold">{t("footer.contact.hours_label")}</span>{" "}
              {t("footer.contact.hours_value")}
            </li>
          </ul>
          ...
          <div className="mt-4 flex items-center gap-3">
            <a
              href="https://www.linkedin.com/company/indoff-mexico/"
              target="_blank"
              rel="noreferrer"
            ...
            <a
              href="https://www.youtube.com/@IndoffUniversity"
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center justify-center h-8 w-8 rounded bg-white/10 hover:bg-white/20 transition"
              aria-label="YouTube"
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2 text-sm">
            <li><a href="/#inicio" className="hover:text-orange-500 transition">{t("footer.links.home")}</a></li>
      ...
      <div className="border-t border-white/10">
        <div className="mx-auto max-w-7xl px-6 py-4 flex flex-col sm:flex-row items-center justify-between text-xs text-gray-400">
          <p>
            © {new Date().getFullYear()} IndoffEdu. {t("footer.bottom.rights")}
          </p>
          <div className="mt-2 sm:mt-0 flex items-center gap-4">
            <a href="#" className="hover:text-orange-500 transition">{t("footer.bottom.terms")}</a>
            <a href="#" className="hover:text-orange-500 transition">{t("footer.bottom.privacy")}</a>
            <a href="/#contacto" className="hover:text-orange-500 transition">{t("footer.bottom.support")}</a>
          </div>
        </div>
      </div>
    </footer>
